...
var Todo = Class({
	...
	constructor: function( data ) {
		this
			// Присваиваем значения по умолчанию и добавляем ключи ``"title"`` and ``"completed"`` в список ключей, отвечающих за данные (см. [jset](http://ru.matreshka.io/#Matreshka.Object-jset)).
			...
			.on( 'render', function( evt ) {
				this
					// Привязка элементов, которые не требуют указания привязчика (binder). Здесь используются стандартные привязчики ([defaultBinders](http://ru.matreshka.io/#Matreshka.Array-defaultBinders)), если возможно.
